...
DELETE FROM sprint_resources; DELETE FROM sprints; DELETE FROM doc_versions; DELETE FROM docs;
...
INSERT INTO docs (slug, section, title, body, current_version, updated_at, updated_by, space) VALUES ('data-model', 'reference', 'Data Model', 'Canopy is a single Cloudflare D1 (SQLite) database. The first-class objects are docs, feed entries, decisions (ADRs), tickets, and sprints; everything else supports them.
...
Identity tables (persons, identities, sessions, mcp_tokens) and the roadmap tables (plan, plan_versions, sprints, sprint_progress) round out the schema. The controlled vocabulary — the sections and tags the gate accepts — lives in its own tables and is the gate''s source of truth.', 1, '2026-06-20T12:00:00Z', 'AndresL230', 'canopy');
INSERT INTO doc_versions (slug, version, body, summary, status, confidence, created_at, created_by) VALUES ('data-model', 1, 'Canopy is a single Cloudflare D1 (SQLite) database. The first-class objects are docs, feed entries, decisions (ADRs), tickets, and sprints; everything else supports them.
...
INSERT INTO docs (slug, section, title, body, current_version, updated_at, updated_by, space) VALUES ('roadmap', 'reference', 'Roadmap', 'The Roadmap is a timeline of sprints — the altitude above individual GitHub issues. Each sprint has a label, a target date, and a status of upcoming, in progress, or done.
...
INSERT INTO doc_versions (slug, version, body, summary, status, confidence, created_at, created_by) VALUES ('roadmap', 1, 'The Roadmap is a timeline of sprints — the altitude above individual GitHub issues. Each sprint has a label, a target date, and a status of upcoming, in progress, or done.
...
INSERT INTO docs (slug, section, title, body, current_version, updated_at, updated_by, space) VALUES ('routes-and-tools', 'reference', 'MCP Tools & HTTP Routes', 'Reads (HTTP, session-gated): GET /feed, /docs, /doc/:slug, /search, /roadmap, /proposals, /needs-triage, /adrs, /auth/me.
...
INSERT INTO docs (slug, section, title, body, current_version, updated_at, updated_by, space) VALUES ('adr-004-live-roadmap', 'decisions', 'ADR-004 · Roadmap progress is computed live, stored nowhere', 'Context: a roadmap whose progress is guessed at render drifts from reality, and a per-viewer GitHub token on the render path makes every page load a network call.
...
Rationale: absolute counts make delivery order irrelevant (the last write wins), and keeping GitHub off the render path means the Roadmap loads from D1 alone and degrades to 0/0 rather than to an error.', 1, '2026-05-20T09:00:00Z', 'AndresL230', 'canopy');
INSERT INTO doc_versions (slug, version, body, summary, status, confidence, created_at, created_by) VALUES ('adr-004-live-roadmap', 1, 'Context: a roadmap whose progress is guessed at render drifts from reality, and a per-viewer GitHub token on the render path makes every page load a network call.

Decision: store the sprints (label, target, status, a bare github_ref) and an ABSOLUTE closed/total cache alongside them, written by the webhook as events arrive and by a scheduled recompute — never at render. A sprint with no cache row reads 0/0. Marking a sprint done stays a deliberate human action — never inferred from 100% closure.

Rationale: absolute counts make delivery order irrelevant (the last write wins), and keeping GitHub off the render path means the Roadmap loads from D1 alone and degrades to 0/0 rather than to an error.', 'Initial published version', 'promoted', 'high', '2026-05-20T09:00:00Z', 'AndresL230');
