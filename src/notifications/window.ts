...
interface LocalDate { year: number; month: number; day: number; weekday: number; } // weekday: 0=Sun..6=Sat

export function localDate(d: Date, timeZone: string): LocalDate {
  const parts = new Intl.DateTimeFormat("en-US", { timeZone, year: "numeric", month: "2-digit", day: "2-digit", weekday: "short" }).formatToParts(d);
