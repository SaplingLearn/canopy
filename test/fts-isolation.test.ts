...
const HARNESS_TRUNCATION =
  "DELETE FROM events; DELETE FROM pr_summaries; DELETE FROM milestone_progress; DELETE FROM plan_versions; UPDATE plan SET narrative = '', current_version = 0, updated_at = NULL, updated_by = NULL; DELETE FROM focus; DELETE FROM milestone_proposals; DELETE FROM milestones; DELETE FROM doc_versions; DELETE FROM docs; DELETE FROM feed; DELETE FROM entry_tags; DELETE FROM adrs; DELETE FROM needs_triage; DELETE FROM sessions; DELETE FROM mcp_tokens; DELETE FROM users;";
...
async function ftsCounts() {
  ...
  return { docs: docs[0].n, feed: feed[0].n, adrs: adrs[0].n };
}

describe("FTS5 migration 0008 — tables, triggers, harness isolation", () => {
  it("creates all three *_fts virtual tables and they start empty (beforeEach truncation left no leak)", async () => {
    const counts = await ftsCounts();
    expect(counts).toEqual({ docs: 0, feed: 0, adrs: 0 });
  });
  ...
  it("AFTER INSERT triggers mirror base rows into the *_fts index", async () => {
    ...
    expect(await ftsCounts()).toEqual({ docs: 1, feed: 1, adrs: 1 });
  });
  ...
  it("the harness truncation statement cascades into *_fts (delete-triggers, no leaked rows)", async () => {
    ...
    expect(await ftsCounts()).toEqual({ docs: 0, feed: 0, adrs: 0 });
  });
